        ...
        <div class="container animate-fade-in">
            ...
            <div id="globalDashboardContainer" style="margin-bottom: 48px;">
                ...
                <div class="skeleton-card" style="padding: 24px; border: 1px solid var(--border-color); box-shadow: var(--shadow-sm);">
                    <div class="skeleton skeleton-chart" style="height: 100px; margin-bottom: 24px;"></div>
...
async function loadGlobalDashboard() {
    ...
    try {
        ...
        if (stats.trend && stats.trend.length > 0) {
            ...
            chartHtml = `
                <div style="flex: 2; min-width: 300px; background: white; border-radius: var(--radius-lg); border: 1px dashed var(--border-color); display: flex; align-items: center; justify-content: center; color: var(--text-muted); min-height: 160px;">
            ...
            <div class="flex gap-2">
                ...
                <div class="dropdown" style="position:relative; display:inline-block;">
                    ...
                    <div id="exportMenu" class="dropdown-content" style="display:none; position:absolute; right:0; top:100%; background:white; box-shadow:var(--shadow-lg); border-radius:var(--radius-md); border:1px solid var(--border-color); min-width: 150px; z-index:10;">
                        <a href="#" onclick="exportToExcel()" style="display:block; padding:12px 16px; color:var(--text-main); text-decoration:none;">Download Excel</a>
                        <a href="#" onclick="exportToPDF()" style="display:block; padding:12px 16px; color:var(--text-main); text-decoration:none; border-top:1px solid var(--border-color);">Download PDF</a>
                    </div>
                </div>
...
function exportToExcel() {
    ...
    if (state.currentStudents.length === 0) return showToast('No data to export', 'error');
    
    const [year, month] = state.currentMonth.split('-');
    const daysInMonth = getDaysInMonth(parseInt(year), parseInt(month));
    
    const aoa = [];
    const header = ['Student Name', 'Parent Email', 'Parent Contact'];
    for(let i=1; i<=daysInMonth; i++) header.push(i.toString());
    header.push('Total Present', 'Total Absent', 'Attendance Rate');
    aoa.push(header);
    
    state.currentStudents.forEach(s => {
        let totalP = 0, totalA = 0;
        const row = [s.name, s.email || '-', s.contact || '-'];
        for(let i=1; i<=daysInMonth; i++) {
            const dateStr = `${year}-${month}-${i.toString().padStart(2, '0')}`;
            const status = s.attendance[dateStr] || '';
            if(status === 'P') totalP++;
        ...
        const total = totalP + totalA;
        row.push(totalP, totalA, total > 0 ? ((totalP/total)*100).toFixed(1)+'%' : '0%');
        aoa.push(row);
    });
    ...
    XLSX.writeFile(wb, `${state.currentClass.name.replace(/\\s+/g, '_')}_${state.currentMonth}.xlsx`);
...
function exportToPDF() {
    ...
    state.currentStudents.forEach(s => {
        ...
        const total = totalP + totalA;
        row.push(totalP.toString(), totalA.toString(), total > 0 ? ((totalP/total)*100).toFixed(0)+'%' : '0%');
        body.push(row);
    });
    ...
    doc.save(`${state.currentClass.name.replace(/\\s+/g, '_')}_${state.currentMonth}.pdf`);
